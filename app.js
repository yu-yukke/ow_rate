...
client.on('message', message => {
  ...
  if (args[0] === 'rate') {
    ...
    if (account === undefined) {
      return message.channel.send('正しい名前で入力するor名前がスクリプトに登録されていないので確認してくださいな');
    }
    ...
    Promise.all([owGetProf, owGetStats])
      .then((result) => {
        ...
        const profile = result[0], stats = result[1];
        const weekday = ["日","月","火","水","木","金","土"];
        const dateToday = new Date();
        const date =
          dateToday.getFullYear() + "年" +
          (dateToday.getMonth() + 1)  + "月" +
          dateToday.getDate() + `日(${weekday[dateToday.getDay()]})`
        const embedStats = new MessageEmbed()
          .setColor('#eaeaea')
          .setTitle(`Stats for ${playerName}`)
          .setURL(`https://playoverwatch.com/ja-jp/career/${platform}/${playerName.replace('#', '-')}`)
          ...
          .addFields(
            ...
            },
            {
              ...
              value:
                 ...
                 `勝率: **${Math.round(profile.games.competitive.win_rate)}%**`
            },
            {
              name: '[使用ヒーロー]',
              value:

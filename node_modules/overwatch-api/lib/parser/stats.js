...
function getHTML(platform, region, tag, callback) {
  var url = platform === 'pc' ? "https://playoverwatch.com/en-us/career/".concat(platform, "/").concat(region, "/").concat(tag, "/") : "https://playoverwatch.com/en-us/career/".concat(platform, "/").concat(tag, "/");
  var options = {
    uri: encodeURI(url),
    encoding: 'utf8'
  };
...
function parseHTML(results, callback) {
  ...
  Object.keys(topHeroCategories.quickplay).forEach(function (k) {
    var topHeroesEls = $("#quickplay [data-category-id=\"".concat(topHeroCategories.quickplay[k], "\"]")).find('.progress-category-item');
    var topHeroes = [];
  ...
  Object.keys(topHeroCategories.competitive).forEach(function (k) {
    var topHeroesEls = $("#competitive [data-category-id=\"".concat(topHeroCategories.competitive[k], "\"]")).find('.progress-category-item');
    var topHeroes = [];
    topHeroesEls.each(function (i, el) {
      var stat = {};
      stat.hero = $(this).find('.ProgressBar-title').text();
  ...
  var statCategories = ['Combat', 'Match Awards', 'Assists', 'Average', 'Miscellaneous', 'Best', 'Game']; // Quickplay Stats.

  statCategories.forEach(function (item) {
    var els = $("#quickplay [data-category-id=\"0x02E00000FFFFFFFF\"] h5:contains(\"".concat(item, "\")")).closest('table').find('tbody tr');
    var statsArr = [];
  ...
  statCategories.forEach(function (item) {
    var els = $("#competitive [data-category-id=\"0x02E00000FFFFFFFF\"] h5:contains(\"".concat(item, "\")")).closest('table').find('tbody tr');
    var statsArr = [];
    els.each(function (i, el) {
      var stat = {};
      stat.title = $(this).find('td').first().text();
...
function transform(results, callback) {
  ...
  var endorsement = {
    sportsmanship: {
      value: parsed.sportsmanshipValue,
      rate: parseFloat((parsed.sportsmanshipValue * 100).toFixed(2))
    },
    shotcaller: {
      value: parsed.shotcallerValue,
      rate: parseFloat((parsed.shotcallerValue * 100).toFixed(2))
    },
    teammate: {
      value: parsed.teammateValue,
      rate: parseFloat((parsed.teammateValue * 100).toFixed(2))
    },
    level: parseInt(parsed.endorsementLevel),
    frame: parsed.endorsementFrame
  };
  endorsement.icon = (0, _svg.createEndorsementSVG)(endorsement); // Calculate the prestige level.

  var level = parsed.level;

  if (parsed.star && parsed.rank) {
    var starsMatch = _path["default"].basename(parsed.star).split('.').slice(0, -1)[0];

    var rankMatch = _path["default"].basename(parsed.rank).split('.').slice(0, -1)[0];
